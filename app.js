// Grid Challenge Game Logic - Enhanced Responsive Version with Progressive Symmetry Difficulty and Level Unlocking
...
class GridChallengeGame {
  constructor() {
    ...
    this.config = {
      ...
      levels: [
        {
          ...
          dotsToMemorize: 2,
          baseTotalDots: 50, // Progressive difficulty - fewer dots
          phases: [
        ...
        },
        {
          ...
          dotsToMemorize: 3,
          baseTotalDots: 70, // Moderate increase
          phases: [
            { type: "highlight", duration: 3 },
            { type: "symmetry", duration: 6 },
            { type: "highlight", duration: 3 },
        ...
        },
        {
          ...
          dotsToMemorize: 4,
          baseTotalDots: 90, // Significant increase
          phases: [
            { type: "highlight", duration: 3 },
            { type: "symmetry", duration: 6 },
            { type: "highlight", duration: 3 },
        ...
        },
        {
          ...
          name: "Level 4 - Expert",
          dotsToMemorize: 5,
          baseTotalDots: 110, // Maximum difficulty
          phases: [
            { type: "highlight", duration: 3 },
            { type: "symmetry", duration: 6 },
            { type: "highlight", duration: 3 },
      ...
      gridConfig: {
        totalDots: 60, // Will be overridden by level-specific calculation
  ...
  // Level Unlocking System Methods
  isLevelUnlocked(level) {
    if (level === 1) return true; // Level 1 is always unlocked

    // Check if previous level has been completed with 50%+ accuracy
    const previousLevel = level - 1;
    const previousLevelData = this.getLevelProgress(previousLevel);

    return previousLevelData.bestAccuracy >= 50;
  }
  ...
  getLevelProgress(level) {
    try {
      ...
      const levelScores = allScores.filter((s) => s.level === level);

      if (levelScores.length === 0) {
        return { bestScore: 0, bestAccuracy: 0, gamesPlayed: 0 };
      }

      const bestScore = Math.max(...levelScores.map((s) => s.score));
      const bestAccuracy = Math.max(...levelScores.map((s) => s.accuracy));
      const gamesPlayed = levelScores.length;

      return { bestScore, bestAccuracy, gamesPlayed };
    } catch (error) {
      console.warn("Could not load level progress:", error);
      return { bestScore: 0, bestAccuracy: 0, gamesPlayed: 0 };
    }
  }
  ...
  updateLevelCardsDisplay() {
    ...
    levelCards.forEach((card, index) => {
      ...
      // Remove any existing lock styling
      card.classList.remove("level-locked");

      if (!isUnlocked) {
        // Add locked styling and disable interaction
        card.classList.add("level-locked");
        card.style.pointerEvents = "none";
        card.style.opacity = "0.5";

        // Update the card content to show lock status
        const lockIndicator =
          card.querySelector(".lock-indicator") || this.createLockIndicator();
        ...
        const requiredProgress = this.getLevelProgress(requiredLevel);
        const currentAccuracy = requiredProgress.bestAccuracy;

        lockIndicator.innerHTML = `
          <div class="lock-icon">🔒</div>
          <div class="lock-text">
            <strong>Locked</strong><br>
            Complete Level ${requiredLevel} with 50%+ accuracy to unlock<br>
            <small>Current Level ${requiredLevel} best: ${currentAccuracy}%</small>
          </div>
        `;
      } else {
        // Remove locked styling and enable interaction
        ...
        const highScoreElement =
          card.querySelector(".high-score") || this.createHighScoreElement();

        if (!card.querySelector(".high-score")) {
          card.querySelector(".level-stats").appendChild(highScoreElement);
        }

        highScoreElement.textContent = `Best Score: ${levelProgress.bestScore}`;
      }
    });
  }
  ...
  showLevelLockedMessage(level) {
    ...
    notification.innerHTML = `
      <div class="notification-content">
        ...
        <p>Complete Level ${requiredLevel} with 50% or higher accuracy to unlock this level.</p>
        <p><strong>Your Level ${requiredLevel} best accuracy: ${currentAccuracy}%</strong></p>
        <button class="btn btn--primary close-notification">Got it!</button>
      </div>
    `;
  ...
  showUnlockProgressMessage() {
    ...
    if (highestUnlocked < 4) {
      ...
      const progressMessage =
        document.getElementById("unlock-progress") ||
        this.createUnlockProgressElement();

      progressMessage.innerHTML = `
        <div class="unlock-progress-content">
          <h3>🎯 Unlock Progress</h3>
          <p>Complete Level ${requiredLevel} with 50%+ accuracy to unlock Level ${nextLevel}</p>
          <div class="progress-bar">
            <div class="progress-fill" style="width: ${Math.min(
              progress.bestAccuracy * 2,
              100
            )}%"></div>
          </div>
          <p><small>Current Level ${requiredLevel} best: ${
      ...
      const progressMessage = document.getElementById("unlock-progress");
      if (progressMessage) {
  ...
  createUnlockProgressElement() {
    const element = document.createElement("div");
    element.id = "unlock-progress";
    element.className = "unlock-progress-message";

    // Insert after level selection grid
    const levelGrid = document.querySelector(".level-selection-grid");
    levelGrid.parentNode.insertBefore(element, levelGrid.nextSibling);

    return element;
  }

  checkForNewUnlocks(currentAccuracy) {
    const currentLevel = this.gameState.selectedLevel;
    const nextLevel = currentLevel + 1;

    // Check if this completion unlocked the next level
    if (nextLevel <= 4 && currentAccuracy >= 50) {
      const wasNextLevelLocked = !this.isLevelUnlocked(nextLevel);

      if (wasNextLevelLocked) {
        // Show unlock notification
        setTimeout(() => {
          this.showLevelUnlockedNotification(nextLevel);
        }, 2000);
      }
    }
  }

  showLevelUnlockedNotification(unlockedLevel) {
    const notification = document.createElement("div");
    notification.className = "level-unlocked-notification";
    notification.innerHTML = `
      <div class="notification-content celebration">
        <h2>🎉 Congratulations!</h2>
        <h3>Level ${unlockedLevel} Unlocked!</h3>
        <p>You achieved 50%+ accuracy and unlocked the next level!</p>
        <button class="btn btn--primary close-notification">Awesome!</button>
      </div>
    `;
  ...
  // Update grid configuration based on screen size and current level
  updateGridConfig() {
    ...
    // Get base total dots for current level
    const currentLevel = this.config.levels[this.gameState.selectedLevel - 1];
    const baseTotalDots = currentLevel ? currentLevel.baseTotalDots : 60;
    ...
    if (screenWidth <= 480) {
      ...
      this.config.gridConfig = {
        totalDots: Math.max(
          15,
          Math.min(Math.floor(baseTotalDots * 0.3), baseTotalDots)
        ),
      ...
      this.config.gridConfig = {
        totalDots: Math.max(
          25,
          Math.min(Math.floor(baseTotalDots * 0.5), baseTotalDots)
        ),
      ...
      this.config.gridConfig = {
        totalDots: Math.max(
          35,
          Math.min(Math.floor(baseTotalDots * 0.65), baseTotalDots)
        ),
      ...
      // Large desktops
      this.config.gridConfig = {
        totalDots: baseTotalDots, // Full count for large screens
    ...
    console.log(
      `Level ${this.gameState.selectedLevel}: Using ${this.config.gridConfig.totalDots} dots (base: ${baseTotalDots})`
    );
  }
  ...
  setupEventListeners() {
    ...
    // Level selection buttons - Updated with unlock check
      ...
      this.elements.levelSelectButtons.forEach((btn) => {
        btn.addEventListener("click", (e) => {
          ...
          if (this.isLevelUnlocked(level)) {
            ...
            this.showLevelLockedMessage(level);
          }
        });
      });
    }
  ...
  showLevelSelection() {
    ...
    // Update level cards to show lock/unlock status
    this.updateLevelCardsDisplay();

    // Show unlock progress message if needed
    this.showUnlockProgressMessage();
  }
  ...
  displayResults() {
    ...
    this.checkForNewUnlocks(successRate);
